...
function fetch_stats(PDO $db): array {
  ...
  if ($res = $db->query("SELECT COUNT(*) AS c FROM ofertas WHERE estado = 'active'")) { $row = $res->fetch(PDO::FETCH_ASSOC); $active = intval($row['c'] ?? 0); $res->closeCursor(); }
  if ($res = $db->query("SELECT COUNT(*) AS c FROM ofertas WHERE estado = 'scheduled'")) { $row = $res->fetch(PDO::FETCH_ASSOC); $scheduled = intval($row['c'] ?? 0); $res->closeCursor(); }
